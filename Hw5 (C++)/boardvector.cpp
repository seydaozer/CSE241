...
namespace Boards{
	...
	void BoardVector::operator=(const BoardVector& other){
		...
		if(row != other.row && column != other.column){
			...
			board.resize(other.row);
			for(int i=0; i<other.row; i++)
				board[i].resize(other.column);
		}
		row = other.row;
		column = other.column;
	...
	string BoardVector::operator()(const int x, const int y) const{
		return board[x][y];
	}

	bool BoardVector::operator==(AbstractBoard& other){
	...
	bool BoardVector::movement(int x, int y){ // this function swaps the two coordinates.
		string temp;
	...
	void BoardVector::setFinalBoard(){ //this function fills the array accordig to the final board.
		calculateWallNumber();
		finalBoard = new string[row*column-wall_number];
	...
	void BoardVector::calculateWallNumber(){ // this function calculates the number of wall in the game.
		int i, j;
		wall_number = 0;
	...
	void BoardVector::initRandomBoard(){
		srand(time(NULL));
	...
	void BoardVector::readFromFile(string file_name){ // this function reads the file.
		setSize(0, 0);
		string line;
		bool flag;
		int size1 = 0, size2 = 0;
	...
	void BoardVector::findEmptyCell(){ // this function find the empty cell's current coordinates.
		int i, j;
		for(i=0; i<row; i++){
			for(j=0; j<column; j++){
				if(board[i][j] == "bb"){
